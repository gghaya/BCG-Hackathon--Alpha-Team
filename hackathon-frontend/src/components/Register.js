...
const Register = () => {
  ...
  const { username, email, password, confirmPassword, isRecruiter } = formData;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await authService.register(username, email, password, isRecruiter);
        ...
        <div className="max-w-lg space-y-8">
          ...
          <div className="space-y-4">
            <h1 className="text-4xl font-display font-bold text-white transition-all duration-500">
              Join BCGX Talent Portal
            </h1>
            <p className="text-xl text-bcg-gray-300 transition-all duration-500">
              Create your account to access exclusive opportunities and connect with BCGX.
            </p>
        ...
        <div>
            ...
            <p className="mt-2 text-center text-sm text-bcg-gray-300 transition-all duration-500">
              ...
              <Link 
                ...
                onMouseEnter={() => setHoveredElement('loginLink')}
                onMouseLeave={() => setHoveredElement(null)}
              >
                Sign in now
            </Link>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
                <label 
                  htmlFor="email" 
                  className={`block text-sm font-medium mb-1 transition-all duration-300 ${
                    activeField === 'email' ? 'text-bcg-mint' : 'text-bcg-gray-300'
                  }`}
                >
                Email address
              </label>
              ...
              <input
                ...
                onChange={handleChange}
                  ...
                  {email.length > 0 && (
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                      <div className={`w-2 h-2 rounded-full transition-all duration-300 ${email.includes('@') && email.includes('.') ? 'bg-bcg-mint' : 'bg-bcg-gray-600'}`}></div>
                    </div>
                  )}
                </div>
            </div>
            ...
            <div>
                <label 
                  ...
                  className={`block text-sm font-medium mb-1 transition-all duration-300 ${
                    activeField === 'confirmPassword' ? 'text-bcg-mint' : 'text-bcg-gray-300'
                  }`}
                >
                Confirm Password
              </label>
                ...
                onChange={handleChange}
                  ...
                  {confirmPassword.length > 0 && (
                    <button
                      ...
                      onMouseEnter={() => setHoveredElement('toggleConfirmPassword')}
                      onMouseLeave={() => setHoveredElement(null)}
                    >
                      {showConfirmPassword ? (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex items-center">
            <input
              ...
              name="isRecruiter"
              type="checkbox"
                  className="h-4 w-4 text-bcg-mint focus:ring-bcg-mint border-bcg-gray-600 rounded transition-colors duration-300"
              checked={isRecruiter}
              onChange={handleChange}
            />
                <label htmlFor="isRecruiter" className="ml-2 block text-sm text-bcg-gray-300 transition-colors duration-300">
